...
# Get current Beijing time from NTP
...
# Wait until the target time, considering ping
def wait_until_target_time(start_beijing_time, start_timestamp):
    next_day = start_beijing_time + timedelta(days=1)
    print(col_y + f"\nRequest for bootloader unlock" + Fore.RESET)
    print (col_g + f"[Set offset]: " + Fore.RESET + f"{feed_time_shift:.2f} ms.")
...
# Check if account unlock is possible via API
def check_unlock_status(session, cookie_value, device_id):
    try:
        ...
        if is_pass == 4:
            ...
            print(message)
            # Write the message to success.txt (without color codes)
            ...
            while True:
                ...
                try:
                    ...
                    if code == 0:
                        ...
                        if apply_result == 1:
                            print(col_g + f"[Status]: " + Fore.RESET + f"The request was approved, checking status...")
                        ...
                        print(col_g + f"[Error]: " + Fore.RESET + f"Response does not contain the required code.")
                        print(col_g + f"[Full response]: " + Fore.RESET + f"{json_response}")

                except json.JSONDecodeError:
                    print(col_g + f"[Error]: " + Fore.RESET + f"Could not decode the JSON from the response..")
